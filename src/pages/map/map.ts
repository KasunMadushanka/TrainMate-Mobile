...
export class MapPage {
    ...
    loadData(callback){
        ...
        this.trains.subscribe(
            trains => {
                trains.map(trains =>
                    coords.push({name:trains.name,latitude:trains.latitude,longitude:trains.longitude,distance:trains.distance,total_distance:trains.total_distance})
                )
                callback(coords);
                coords=[];
            });
                ...
                this.map.on(GoogleMapsEvent.MAP_READY).subscribe(() => {
                    ...
                    for(let i=0;i<coords.length;i++){
                        ...
                        let markerOptions: GoogleMapsMarkerOptions = {
                            position: l,
                            title: coords[i].name,
                            icon:'http://trainmate03.azurewebsites.net/images/train.png',
